...
async function getYahooItemData(url) {
    ...
    if (pageDataMatch) {
        try {
            ...
            if (data.items) {
                ...
                price = parseInt(data.items.price, 10).toLocaleString('ja-JP') + '蜀・;
    ...
    if (!title) {
        ...
        title = titleMatch ? titleMatch[1].replace(' - Yahoo!繧ｪ繝ｼ繧ｯ繧ｷ繝ｧ繝ｳ', '').replace(' - 繝､繝輔が繧ｯ!', '').trim() : '';
    }

    if (html.includes('縺薙・繧ｪ繝ｼ繧ｯ繧ｷ繝ｧ繝ｳ縺ｯ邨ゆｺ・＠縺ｦ縺・∪縺・) || html.includes('繧ｪ繝ｼ繧ｯ繧ｷ繝ｧ繝ｳ邨ゆｺ・)) {
        isClosed = true;
    }

    const statusText = isClosed ? '谺蜩・ : '雋ｩ螢ｲ荳ｭ';
    ...
    try {
        ...
        const info = await page.evaluate((targetUrl) => {
            ...
            if (targetUrl.includes('amazon.co.jp')) {
                ...
                if (price && !price.includes('蜀・) && price.includes('・･')) {
                    price = price.replace('・･', '') + '蜀・;
                }

                const availabilityEl = document.querySelector('#availability');
                const availText = availabilityEl ? availabilityEl.textContent.trim() : '';
                isClosed = availText.includes('荳譎ら噪縺ｫ蝨ｨ蠎ｫ蛻・ｌ') || availText.includes('迴ｾ蝨ｨ縺雁叙繧頑桶縺・＠縺ｦ縺翫ｊ縺ｾ縺帙ｓ') || availText.includes('蝨ｨ蠎ｫ蛻・ｌ');
                ...
                if (rawPrice.includes('蜀・)) {
                    const priceMatch = rawPrice.match(/([0-9,]+蜀・/);
                ...
                isClosed = Boolean(soldBadge || (checkoutBtn && checkoutBtn.disabled && btnText.includes('螢ｲ繧雁・繧・)));
            }
        ...
        return { title: '蜿門ｾ励お繝ｩ繝ｼ', price: '', isClosed: false, statusText: '雋ｩ螢ｲ荳ｭ' };
    }
}
...
(async () => {
    ...
    async function setCellRedBackground(cellName) {
        ...
        await page.evaluate(() => {
            const swatches = Array.from(document.querySelectorAll('.docs-material-colorpalette-colorswatch, [aria-label*="襍､"], [title*="襍､"], [data-color="#f44336"], [data-color="#ff0000"], [data-color="#ea4335"]'));
            for (const s of swatches) {
                const label = s.getAttribute('aria-label') || s.getAttribute('title') || '';
                const color = s.getAttribute('data-color') || '';
                if (label.includes('襍､') || color === '#f44336' || color === '#ff0000' || color === '#ea4335') {
                    s.click();
                    return true;
                }
            }
    ...
    for (let r = 2; r < 100; r++) {
        ...
        // CHECK IF 谺蜩・(SOLDOUT/Closed)
        if (itemData.statusText === '谺蜩・) {
            console.log(`Row ${r} is 谺蜩・(SOLDOUT). Writing '谺蜩・ into C${r}, E${r}, F${r}, and '蜃ｺ蜩∝叙繧頑ｶ医＠' into G${r}.`);
            await selectCell(`C${r}`);
            await overwriteCellText('谺蜩・);
            ...
            await overwriteCellText('蜃ｺ蜩∝叙繧頑ｶ医＠');
            continue;
        }

        // If 雋ｩ螢ｲ荳ｭ (Active):
        console.log(`Row ${r} is 雋ｩ螢ｲ荳ｭ. Writing product name to C${r}...`);
        await selectCell(`C${r}`);
        await overwriteCellText(itemData.title);

        console.log(`Writing status '雋ｩ螢ｲ荳ｭ' to F${r}...`);
        await selectCell(`F${r}`);
        await overwriteCellText('雋ｩ螢ｲ荳ｭ');
