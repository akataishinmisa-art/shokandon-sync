...
async def test_mercari(keyword, sort_mode, out_file):
    ...
    async with async_playwright() as p:
        ...
        out_file.write(f"Total raw items on page: {len(items_raw)}\n")
        found_target = False
        for it in items_raw:
            raw = it.get("raw_text", "").replace("\n", " ")
            url_item = it.get("url", "")
            title = it.get("title", "")
            out_file.write(f"Item: {raw} | URL: {url_item}\n")
            if "34,225" in raw or "34225" in raw:
                out_file.write(f"  >>> FOUND 34,225 ITEM! Title: {title}\n")
                found_target = True

        if not found_target:
            out_file.write("  >>> 34,225 item NOT found on this search.\n")

async def main():
    with open("mercari_search_result.txt", "w", encoding="utf-8") as f:
        f.write("Testing recent mode:\n")
        await test_mercari("EX-ZR1800", "recent", f)
        f.write("\nTesting all mode (price desc):\n")
        await test_mercari("EX-ZR1800", "all", f)
        f.write("\nTesting CASIO EX-ZR1800 recent mode:\n")
        await test_mercari("CASIO EX-ZR1800", "recent", f)
