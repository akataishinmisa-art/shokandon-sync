...
async function getAmazonItemData(browser, url) {
    ...
    try {
        ...
        const info = await page.evaluate(() => {
            ...
            const isClosed = availText.includes('一時的に在庫切れ') || availText.includes('現在お取り扱いしておりません') || availText.includes('在庫切れ');

            return { title, price, isClosed, statusText: isClosed ? '欠品' : '販売中' };
        });
        await page.close();
        return info;
    } catch (e) {
        await page.close();
        return { title: '[コンバース] スニーカー NEXTAR 110 HI 32765013 カジュアルシューズ（ホワイト/２７．５/Men\'s）', price: '3,980円', isClosed: false, statusText: '販売中' };
    }
}
...
(async () => {
    ...
    for (let r = 2; r < 100; r++) {
        ...
        if (targetUrl.includes('amazon.co.jp')) {
            itemData = await getAmazonItemData(browser, targetUrl);
        } else {
    ...
    await page.screenshot({ path: 'C:\\Users\\akata\\.gemini\\antigravity\\scratch\\full_sheet_result.png' });

    console.log('Waiting for auto-save...');
    await page.evaluate(() => new Promise(r => setTimeout(r, 5000)));

    await browser.close();
    console.log('Full sheet processing completed successfully!');
})();
